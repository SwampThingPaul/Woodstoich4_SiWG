...
#Export data to xslx
meta=data.frame(Notes=c("This file provides average values for each Pool (FLDNUM) and STRATUM across each pool (Pool tab), roughly USGS season (Seasonal), deade and annual timesteps (Decade and Annual tabs, respectively). ",
                        "The source data file UMR_MainStem_SRSdataset_1993-2018.csv was used to compute mean values.",
                        "Fatally qualified data were removed and TP (TP vs SRP) and TN (TN vs DIN) reversals were identified and removed prior to analysis.",
                        "Data was aggregated using the 'umesc_mean.R' script located on the project GitHub Repositoty (https://github.com/SwampThingPaul/Woodstoich4_SiWG)"))
xlsx::write.xlsx(meta,paste0(export.path,"SRS_summarystats.xlsx"),sheetName="Metadata",row.names = F,showNA=F)
...
xlsx::write.xlsx(merge(season,parameter.units,"parameter"),paste0(export.path,"SRS_summarystats.xlsx"),sheetName="Seasonal",row.names = F,showNA=F,append=T)
...
annual.fixed=ddply(dat.xtab.fix.melt,c("CY","LOCATCD","dataset","FLDNUM","parameter"),summarise,mean.val=mean(value,na.rm=T),sd.val=sd(value,na.rm=T),N.val=N(value))
decade.fixed=ddply(dat.xtab.fix.melt,c("decade","LOCATCD","dataset","FLDNUM","parameter"),summarise,mean.val=mean(value,na.rm=T),sd.val=sd(value,na.rm=T),N.val=N(value))
season.fixed=ddply(dat.xtab.fix.melt,c("season","LOCATCD","dataset","FLDNUM","parameter"),summarise,mean.val=mean(value,na.rm=T),sd.val=sd(value,na.rm=T),N.val=N(value))
station.fixed=ddply(dat.xtab.fix.melt,c("LOCATCD","dataset","FLDNUM","parameter"),summarise,mean.val=mean(value,na.rm=T),sd.val=sd(value,na.rm=T),N.val=N(value))

parameter.units=data.frame(parameter=vars,units=c(rep("mg/L",5),rep("mol:mol",6)))

#Export data to xslx
meta.fixed=data.frame(Notes=c("This file provides average values for each Station (LOCATCD), roughly USGS season (Seasonal), deade and annual timesteps (Decade and Annual tabs, respectively) for tributary and mainstem datasets (dataset field). ",
                        "The source data file 'UMR_Tribs_1988-2018.csv' and 'UMR_Mainstem_1991-2018.csv' was used to compute mean values.",
                        "Fatally qualified data were removed and TP (TP vs SRP) and TN (TN vs DIN) reversals were identified and removed prior to analysis.",
                        "Data was aggregated using the 'umesc_mean.R' script located on the project GitHub Repositoty (https://github.com/SwampThingPaul/Woodstoich4_SiWG)"))

xlsx::write.xlsx(meta.fixed,paste0(export.path,"Fixed_summarystats.xlsx"),sheetName="Metadata",row.names = F,showNA=F)
xlsx::write.xlsx(merge(station.fixed,parameter.units,"parameter"),paste0(export.path,"Fixed_summarystats.xlsx"),sheetName="Station",row.names = F,showNA=F,append=T)
xlsx::write.xlsx(merge(season.fixed,parameter.units,"parameter"),paste0(export.path,"Fixed_summarystats.xlsx"),sheetName="Seasonal",row.names = F,showNA=F,append=T)
xlsx::write.xlsx(merge(decade.fixed,parameter.units,"parameter"),paste0(export.path,"Fixed_summarystats.xlsx"),sheetName="Decade",row.names = F,showNA=F,append=T)
xlsx::write.xlsx(merge(annual.fixed,parameter.units,"parameter"),paste0(export.path,"Fixed_summarystats.xlsx"),sheetName="Annual",row.names = F,showNA=F,append=T)
